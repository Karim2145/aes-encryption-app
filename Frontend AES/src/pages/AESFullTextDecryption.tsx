...
type OutputFormat = "hex" | "binary" | "decimal" | "ascii";

const defaultKey128 = "2b7e151628aed2a6abf7158809cf4f3c";
const LAST_KEY_STORAGE = "aes:lastKey";
const LAST_KEY_SIZE_STORAGE = "aes:lastKeySize";

const AESFullTextDecryption: React.FC = () => {
  ...
  /* -------------------- ciphertext & result -------------------- */
  const [fullCiphertext, setFullCiphertext] = useState("");
  const [uploadedFileName, setUploadedFileName] = useState<string | null>(null);
  const [multiError, setMultiError] = useState<string | null>(null);
  const [multiResult, setMultiResult] =
    useState<MultiBlockDecryptionResult | null>(null);
  ...
  useEffect(() => {
    ...
    const firstBlock = multiResult.blockResults[0];

    setVisualResult(firstBlock.aesResult);
    setVisualLabel(
      `${multiResult.mode} mode — block 1 of ${multiResult.blockResults.length}`
    );
  ...
  const persistKeyForSharing = (): string | null => {
    ...
    if (!normalizedKey) {
      ...
      sessionStorage.removeItem(LAST_KEY_SIZE_STORAGE);
      return null;
    }
    sessionStorage.setItem(LAST_KEY_STORAGE, normalizedKey);
    sessionStorage.setItem(LAST_KEY_SIZE_STORAGE, keySize.toString());
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      doc.text(
        `AES Full Text Decryption - ${multiResult.mode} Mode`,
        pageWidth / 2,
        yPos,
        {
          align: "center",
      ...
      doc.save(
        `AES-${keySize}_${multiResult.mode}_decryption_block-${
  ...
  const downloadDecryptedFile = () => {
    ...
    try {
      const plaintextHex = multiResult.plaintextHex;
      const bytes = hexToBytes(plaintextHex);
      ...
      switch (outputFormat) {
        case "hex":
          content = plaintextHex.toUpperCase();
          suffix = "hex";
          break;
        case "binary":
          content = bytesToBinary(bytes);
          suffix = "binary";
          break;
        case "decimal":
          content = bytesToDecimal(bytes);
          suffix = "decimal";
          break;
        case "ascii":
        default:
      ...
      a.href = url;
      a.download = `decrypted_${uploadedFileName || "text"}_${mode}_${suffix}.txt`;
  ...
  /* -------------------- full TEXT decryption (BACKEND) -------------------- */

  const handleDecryptFullText = async () => {
    try {
      setMultiError(null);

      if (!fullCiphertext.trim()) {
        throw new Error("Ciphertext is empty.");
      }

      if (!hexOnlyRegex.test(fullCiphertext)) {
        throw new Error("Ciphertext must be hexadecimal (0–9, A–F) only.");
      }

      const normalizedCiphertext = fullCiphertext.replace(/\s+/g, "");
      if (normalizedCiphertext.length % 2 !== 0) {
        throw new Error(
          "Ciphertext hex length must be even (two hex characters per byte)."
        );
      }
      ...
      const payload = {
        ciphertextHex: normalizedCiphertext,
        keyHex: normalizedKey,
        keySize,
        mode,
        ivHex:
          mode === "CBC" || mode === "CFB" || mode === "OFB"
            ? ivHex.replace(/\s+/g, "") || null
            : null,
        ctrCounterHex:
          mode === "CTR" ? ctrCounterHex.replace(/\s+/g, "") || null : null,
      };
      ...
      showToast("Ciphertext decrypted via backend.");
    } catch (err: any) {
      setMultiError(err.message ?? "Full-text decryption failed.");
      setMultiResult(null);
      setVisualResult(null);
    }
  };
  ...
  /* -------------------- navigation to encryption with key-copy prompt -------------------- */

  const handleNavigateToEncryption = async () => {
    const normalizedKey = persistKeyForSharing();
    if (normalizedKey && !keyCopied) {
      const shouldCopy = window.confirm(
        "You haven't copied this key yet. Copy it before switching to encryption?"
      );
      if (shouldCopy) {
        await handleCopy(normalizedKey, "Key");
        setKeyCopied(true);
      }
    }
    navigate("/aes/full-text");
  };
  ...
  return (
    <div
      className={`aes-root ${
        theme === "dark" ? "aes-root--dark" : "aes-root--light"
      }`}
    >
      <UserDropdown />

      <header
        className="aes-header"
        style={{
      ...
      >
        <div className="aes-view-switcher">
          <button
            type="button"
          ...
          <button
            ...
            className="aes-button aes-button--primary"
            style={{ marginRight: "8px" }}
            onClick={handleNavigateToEncryption}
          >
            Go to encryption →
          </button>
        </div>
        ...
        >
          ...
          <div>
            <h1 className="aes-title">Full Text AES Decryption</h1>
            <p className="aes-subtitle">
              Decrypt any ciphertext using AES-128/192/256 in 5 modes, with
      ...
      <main className="aes-layout">
        ...
        <section className="aes-panel aes-panel--left">
              ...
              <div className="aes-input-row">
                ...
                <button
                  ...
                  onClick={async () => {
                    const normalizedKey = persistKeyForSharing();
                    if (!normalizedKey) {
                      showToast("Enter a key before copying");
                      return;
                    }
                    await handleCopy(normalizedKey, "Key");
                    setKeyCopied(true);
          ...
          <div className="aes-section">
            ...
            <div className="aes-form-group">
              <label className="aes-label">
                Ciphertext (hex, any length)
                <textarea
                  className="aes-textarea"
                  rows={4}
                  value={fullCiphertext}
                  onChange={(e) => setFullCiphertext(e.target.value)}
                  placeholder="Paste ciphertext in hex format or upload a file..."
                />
              </label>
              <p className="aes-help">
                Characters: {fullCiphertext.replace(/\s+/g, "").length} |
                Estimated blocks:{" "}
                {Math.ceil(
                  fullCiphertext.replace(/\s+/g, "").length / 32
                ) || 0}
              </p>
            </div>
        ...
        <section className="aes-panel aes-panel--right">
            ...
            <div>
              ...
              {hasVisualSteps && currentStep ? (
                <p className="aes-steps-subtitle">
                  {visualLabel} — round{" "}
                  <strong>{currentStep.round}</strong>, step{" "}
                  {currentStepIndex + 1} of {totalSteps}:{" "}
                  <span className="aes-steps-stepname">{currentStep.step}</span>
                </p>
              ) : (
                <p className="aes-steps-subtitle">
                  Decrypt ciphertext to visualize a block.
                </p>
              )}
            </div>
          ...
          <div className="aes-cipher-section">
            <div className="aes-cipher-header">
              <span className="aes-result-label">
            ...
            <div
              className="aes-cipher-value"
              style={{ wordBreak: "break-all" }}
            >
              {visualResult
                ? formatOutput(visualResult.ciphertext)
                : "No plaintext yet — run decryption first."}
            </div>
          </div>
                ...
                >
                  {multiResult.blockResults.map((blk) => (
                    <option key={blk.blockIndex} value={blk.blockIndex}>
                      Block {blk.blockIndex + 1} — ciphertext hex{" "}
                      {blk.ciphertextBlockHex.toUpperCase()}
                    </option>
                  ))}
                </select>
              </label>
                ...
                <div className="aes-legend-item aes-legend-item--state">
                  ...
                  <div>
                    ...
                    <div className="aes-legend-text">
                      4×4 bytes representing the internal AES state during
                      decryption.
                    </div>
                  </div>
                </div>
...
export default AESFullTextDecryption;
